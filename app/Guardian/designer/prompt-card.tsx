...
export const PromptCard: React.FC<PromptCardProps> = ({ 
  ...
  return (
    <div 
      onClick={onSelect}
      className={`
        border p-3 md:p-4 flex flex-col h-full relative group transition-all duration-300 cursor-pointer
        ${isSelected 
          ? 'border-[#00ffff] bg-[#001111] shadow-[0_0_20px_#00ffff40] scale-[1.01] md:scale-[1.02]' 
    ...
    >
      {/* Selection Marker */}
      {isSelected && (
        <div className="absolute -top-3 -right-3 bg-[#00ffff] text-black text-[10px] font-bold px-2 py-1 z-20">
          SELECTED
        </div>
      )}
      ...
      <div className="mb-4 border-b border-[#004444] pb-2 flex justify-between items-center">
        <h3 className={`text-base md:text-lg font-bold tracking-widest ${isSelected ? 'neon-text' : 'text-[#008888]'}`}>{title}</h3>
        {imageStatus === 'LOADING' && (
      ...
      <div className={`w-full aspect-square mb-4 border border-dashed border-[#004444] flex items-center justify-center bg-[#000505] overflow-hidden relative transition-colors ${imageStatus === 'IDLE' ? 'group-hover:border-[#008888]' : 'border-[#00ffff]'}`}>
        ...
        {imageStatus === 'IDLE' && (
          <span className="text-[#004444] text-[10px] group-hover:text-[#008888] transition-colors">
            [IMG_PLACEHOLDER_404]
          </span>
        )}
      ...
      <div className="flex-grow mb-4 overflow-y-auto max-h-32 md:max-h-48 pr-2 scrollbar-thin">
        <p className="text-[10px] md:text-xs leading-relaxed text-[#ccffff] opacity-90 font-light">
          {promptText}
        </p>
      </div>
      ...
      <button
        onClick={handleCopy}
        className={`
          w-full py-4 px-4 text-xs font-bold uppercase tracking-wider
